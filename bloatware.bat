...
:: Get the list of preinstalled apps
for /f "tokens=2,*" %%A in ('reg query "HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall" /s ^| findstr DisplayName') do (
    ...
    :: Check if the app is necessary for system functionality
...
set "uninstallApps="
set /p "uninstallApps=Enter the app(s) you want to uninstall (comma-separated, leave blank to exit): "

:: Check if the user provided any apps to uninstall
if not "%uninstallApps%"=="" (
    :: Split the input into individual app names
